...
const Tips: React.FC = () => {
  ...
  // Helper para mapear string de categoria para ícone e cor
  const getTipTheme = (category: string) => {
    const cat = category.toLowerCase();
    if (cat.includes('nutrição') || cat.includes('alimentação')) return { icon: Apple, color: 'bg-green-100 text-green-600' };
    if (cat.includes('exame') || cat.includes('médico') || cat.includes('saúde')) return { icon: Activity, color: 'bg-blue-100 text-blue-600' };
    ...
    return { icon: Brain, color: 'bg-rose-100 text-rose-600' };
  };

  // Sort tips by week
  const sortedTips = [...tips].sort((a, b) => a.minWeek - b.minWeek);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <div className="text-center md:text-left">
            <h2 className="text-2xl font-bold text-slate-800">Guia da Gestação</h2>
            <p className="text-slate-500 mt-2 max-w-xl">
              Acompanhe as principais recomendações para cada etapa da sua jornada.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedTips.map((tip) => {
          const theme = getTipTheme(tip.category);
          const Icon = theme.icon;

          return (
            <div key={tip.id} className="bg-white flex flex-col p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-teal-200 transition-all group cursor-default relative overflow-hidden">
